...
uint8_t nodeId = 1;      // requires amending for each node
...
void setup()
{
  ...
  nodeId = 1; // requires amending for each node
  if (nodeId > 10)
  {
...
void updateRoutingTable()
{
  for (uint8_t n = 1; n <= N_NODES; n++)
  {
    ...
    void
    getRouteInfoString(char *p, size_t len)
{
  p[0] = '\0';
  strcat(p, "[");
  for (uint8_t n = 1; n <= N_NODES; n++)
...
void loop()
{
  ...
  {
    if (n == nodeId)
      continue; // self so ignore this route
